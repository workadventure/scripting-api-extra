...
export const variablesStore = derived<Readable<ITiledMapLayer|undefined>, Map<string, VariableDescriptor>|undefined>(configurationLayerStore, ($configurationLayerStore, set) => {
    ...
    if (!!hash) {
        ...
        getVariables(undefined, variablesToKeep).then((variables) => {
            set(variables);
        }).catch(e => console.error(e))
    } else if ($configurationLayerStore) {
        // Mostly for the global configuration panel with sections
        getVariables($configurationLayerStore.name).then((variables) => {
            set(variables);
        }).catch(e => console.error(e))
    } else {
